...
#define DEBUG_EMUL_BEGIN() \
	do\
    {\
		LOGD(LOG_COLOR_CYAN"[file = %s : method = %s : line = %d] BEGIN >>>>"LOG_COLOR_END, basename(__FILE__), __func__, __LINE__); \
    } while( 0 )

#define DEBUG_EMUL_END() \
	do\
    {\
		LOGD(LOG_COLOR_CYAN"[file = %s : method = %s : line = %d] END >>>>"LOG_COLOR_END, basename(__FILE__), __func__, __LINE__); \
    } \
    while( 0 )
